...
SCENARIOS = [
    ...
    },
    {
        "name": "Burry Bust",
        "probability": 10,
        "description": "Circular financing unwind: ~2/3 of AI trifecta spend is circular (chipmakers fund labs, labs buy chips, hyperscalers fund labs, labs buy cloud). A major lab misses revenue targets by >30%, triggering cascade. GPU spot prices fall 40–60%. GPU-collateralised debt faces collateral impairment. Chip equity sells off 30–50%. Power equipment less affected (non-cancellable order books). $1.65T in off-balance-sheet SPV/lease debt (MSFT/GOOGL/AMZN/META/ORCL) traces via private credit (Blue Owl, PIMCO, Apollo, BlackRock) to PE-owned life insurers (Athene, Global Atlantic), with state guaranty funds as the ultimate backstop if defaults exceed ~15% of the private-credit loan book.",
        "implications": ["GPU spot falls to $1.00–1.50/hr; NVIDIA DC revenue $90–120B", "CoreWeave restructuring — GPU-collateralised debt impaired; 5yr CDS ~855bps already implies ~50% cumulative default probability (Aug 2026)", "Off-balance-sheet debt ($1.65T) is decoupled from hyperscaler balance sheets — risk concentrates in private credit funds and PE-owned insurers, not visible in headline hyperscaler leverage ratios", "Power equipment resilient on signed non-cancellable backlogs", "Key test: monitor non-circular revenue ratio across AI trifecta, hyperscaler bond coverage ratios (already compressed 5x→2x, Feb→Jul 2026), and CDS spreads on CoreWeave/Oracle"],
        "color": "#7c3aed",
    },
    {
